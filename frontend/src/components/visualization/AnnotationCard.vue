<template>
  <div
    class="flex flex-col rounded-[10px] border border-secondary/15 bg-white/70"
    :class="selected ? 'ring-1 ring-primary/40' : ''"
  >
    <div class="flex flex-col gap-2 p-3">
      <div class="flex items-start gap-1.5">
        <button
          ...
          :aria-label="
            expanded
              ? t('fomcharts.annotations.collapseDetails')
              : t('fomcharts.annotations.expandDetails')
          "
          @click="$emit('toggle-expand')"
        >
          <span class="flex items-center gap-1.5">
            <span class="shrink-0 font-mono text-xs font-bold text-primary">{{
              note.ref
            }}</span>
          ...
          <span
            v-if="axisBadges.length"
            class="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-[10.5px] text-secondary"
          >
            <span v-for="badge in axisBadges" :key="badge.key"
              >{{ badge.key }}
              <b class="font-mono text-ink">{{ badge.value }}</b></span
            >
          </span>
        </button>
        <Tooltip>
      ...
      <div class="flex items-center justify-between gap-2">
        ...
        >
          ...
          <span class="truncate">{{
            compareLimitReached
              ? t("fomcharts.annotations.compareLimitReached", {
                  max: compareMax,
                })
    ...
    >
      <div class="min-h-0 overflow-hidden">
        <div class="flex flex-col gap-2.5 px-3.5 pb-3.5">
          <div
            v-if="originField"
            class="grid grid-cols-[auto_1fr] gap-x-2.5 rounded-md border border-secondary/20 bg-card px-2.5 py-2 text-xs"
          >
          ...
          >
            <div
              v-if="leadingFields.length"
              class="grid grid-cols-[auto_1fr] gap-x-2.5 gap-y-1.5 text-xs"
            >
            ...
            <div v-if="modeDescription">
              ...
              <div
                class="grid transition-[grid-template-rows] duration-200 ease-out"
                :style="{
                  gridTemplateRows: descriptionExpanded ? '1fr' : '0fr',
                }"
              >
                <div class="min-h-0 overflow-hidden">
                  <p class="pt-1 font-mono text-xs text-secondary">
                    {{ modeDescription }}
                  </p>
                </div>
              </div>
          ...
          <div
            v-if="initialLayers.length > 0"
            class="flex flex-col gap-1 rounded-md border border-secondary/20 bg-card px-2 py-1.5"
          >
            <div class="flex items-center justify-between gap-2">
              <span class="truncate text-[10.5px] text-secondary"
              ...
              <Dialog>
                ...
                <DialogContent class="max-w-sm">
                  ...
                  <div
                    v-if="structureExtraFields.length"
                    class="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5 text-xs"
                  >
                    <template v-for="f in structureExtraFields" :key="f.key">
                      <span class="text-muted-foreground">{{ f.key }}</span>
                      <span
                        class="font-mono font-semibold wrap-break-word text-ink"
                        >{{ f.value }}</span
                      >
                    </template>
                  </div>
            ...
            >
              <p
                v-for="f in structureExtraFields"
                :key="f.key"
                class="truncate text-[10px] text-muted-foreground"
              >
                {{ f.key }}
                <b class="font-mono text-secondary">{{ f.value }}</b>
              </p>
            </div>
          </div>

          <div
            v-if="exportMetricsRows.length"
            class="flex flex-col gap-1 rounded-md border border-secondary/20 bg-card px-2 py-1.5"
          >
            <div class="flex items-center justify-between gap-2">
              <span class="truncate text-[10.5px] text-secondary"
                >{{ t("fomcharts.annotations.metrics") }} ({{
                  exportMetricsRows.length
                }})</span
              >
              <Dialog>
                <Tooltip>
                  ...
                  <TooltipContent>{{
                    t("fomcharts.annotations.zoomMetrics")
                  }}</TooltipContent>
                </Tooltip>
                <DialogContent class="max-w-sm">
                  <DialogTitle class="pr-6">
                  ...
                  <div
                    class="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-xs"
                  >
          ...
          >
            <div class="flex items-center justify-between gap-2">
              <span class="text-[10.5px] text-secondary">{{
              ...
              <Dialog>
                      ...
                      <button
                        ...
                        :class="
                          note.note
                            ...
                            : 'text-muted-foreground hover:bg-secondary/10 hover:text-ink'
                        "
                ...
                <DialogContent class="max-w-sm">
                  ...
                  <Textarea
                    ...
                    rows="6"
                    @update:model-value="
                      (value) => $emit('update-note', String(value))
                    "
                  />
                  <div class="mt-3 flex justify-end">
                    <Button
            ...
            <p
              v-if="note.note"
              class="line-clamp-2 font-mono text-[10px] text-muted-foreground"
            >
              {{ note.note }}
            </p>
          </div>
...
const structureExtraFields = computed(
  () => cardData.value.structureExtraFields,
);
...
const downloadLayersPng = () =>
  exportLayerStackPng(
    ...
    props.initialLayers,
    structureExtraFields.value,
  );
const downloadFieldsPng = () =>
  exportFieldListPng(
    { ref: props.note.ref, title: props.note.title },
    exportMetricsRows.value,
  );
const downloadNoteTxt = () =>
  downloadTextFile(
    props.note.note,
    `notes_${props.note.ref.replace(/[^a-z0-9_-]+/gi, "_")}.txt`,
  );
...
const foldFieldsPreview = computed(() =>
  exportMetricsRows.value.map((f) => f.value).join(" · "),
);

const buildExportSections = (): AnnotationExportSection[] => [
  {
    title: t("fomcharts.annotations.mode"),
    rows: props.leadingFields,
    text: props.modeDescription ?? undefined,
  },
  {
  ...
  { title: t("fomcharts.annotations.metrics"), rows: exportMetricsRows.value },
  {
    ...
    text: props.note.note || undefined,
  },
];
